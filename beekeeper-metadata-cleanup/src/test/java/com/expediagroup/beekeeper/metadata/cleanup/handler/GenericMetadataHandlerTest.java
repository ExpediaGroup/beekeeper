...
public class GenericMetadataHandlerTest {
  ...
  public void typicalRunDroppingTable() {
    ...
    Pageable pageable = handler.processPage(mockPageable, CLEANUP_INSTANCE, mockPage, false);
  ...
  public void typicalDryRunDroppingTable() {
    ...
    when(
        housekeepingMetadataRepository.countRecordsForDryRunWherePartitionIsNotNullOrExpired(CLEANUP_INSTANCE, DATABASE, TABLE_NAME))
        .thenReturn(Long.valueOf(0));
    when(metadataCleaner.tableExists(DATABASE, TABLE_NAME)).thenReturn(true);

    Pageable pageable = handler.processPage(mockPageable, CLEANUP_INSTANCE, mockPage, true);
    verify(metadataCleaner).dropTable(mockMetadata);
    verify(pathCleaner).cleanupPath(mockMetadata);
    verify(metadataCleaner, never()).dropPartition(mockMetadata);
