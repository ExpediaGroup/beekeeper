...
public abstract class GenericMetadataHandler {
  ...
  private void cleanUpTable(EntityHousekeepingPath housekeepingPath) {
    MetadataCleaner metadataCleaner = getMetadataCleaner();
    metadataCleaner.cleanupMetadata(housekeepingPath);
  ...
  private void cleanupContent(EntityHousekeepingPath housekeepingPath) {
    try {
      log.info("Cleaning up metadata \"{}.{}\"", housekeepingPath.getDatabaseName(), housekeepingPath.getTableName());
      cleanUpTable(housekeepingPath);

      // TODO
      // change to housekeeping status ???
      ...
      log
          .warn("Unexpected exception deleting metadata \"{}.{}\"", housekeepingPath.getDatabaseName(),
              housekeepingPath.getTableName(), e);
    }
  }
