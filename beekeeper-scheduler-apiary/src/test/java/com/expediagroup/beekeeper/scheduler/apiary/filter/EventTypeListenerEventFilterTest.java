...
public class EventTypeListenerEventFilterTest {
  ...
  public void typicalFilterAlterTableEvent() {
    ...
    boolean filter = listenerEventFilter.isFiltered(alterTableEvent, UNREFERENCED);
    assertThat(filter).isFalse();
  }

  @Test
  public void typicalFilterDropPartitionEvent() {
    when(dropPartitionEvent.getEventType()).thenReturn(DROP_PARTITION);
    boolean filter = listenerEventFilter.isFiltered(dropPartitionEvent, UNREFERENCED);
    assertThat(filter).isFalse();
  }
  ...
  public void typicalFilterDropTableEvent() {
    ...
    boolean filter = listenerEventFilter.isFiltered(dropTableEvent, UNREFERENCED);
    assertThat(filter).isFalse();
  }

  @Test
  public void typicalFilterOtherEvent() {
    when(addPartitionEvent.getEventType()).thenReturn(ADD_PARTITION);
    boolean filter = listenerEventFilter.isFiltered(addPartitionEvent, UNREFERENCED);
