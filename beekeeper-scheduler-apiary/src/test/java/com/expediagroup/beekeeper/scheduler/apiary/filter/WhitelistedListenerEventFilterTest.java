...
public class WhitelistedListenerEventFilterTest {
  ...
  public void filterDefaultEvents(EventType eventType) {
    ...
    boolean filter = listenerEventFilter.isFiltered(listenerEvent, UNREFERENCED);
  ...
  public void filterWhitelistedEvent(String whitelist) {
    when(listenerEvent.getEventType()).thenReturn(DROP_TABLE);
    when(listenerEvent.getTableParameters()).thenReturn(Map.of(BEEKEEPER_HIVE_EVENT_WHITELIST, whitelist));
    boolean filter = listenerEventFilter.isFiltered(listenerEvent, UNREFERENCED);
    assertThat(filter).isFalse();
  }
  ...
  public void filterNullTableParametersDefaultEvent() {
    when(listenerEvent.getEventType()).thenReturn(ALTER_TABLE);
    when(listenerEvent.getTableParameters()).thenReturn(null);
    boolean filter = listenerEventFilter.isFiltered(listenerEvent, UNREFERENCED);
    assertThat(filter).isFalse();
  }
  ...
  public void filterEmptyTableParametersDefaultEvent() {
    when(listenerEvent.getEventType()).thenReturn(ALTER_TABLE);
    when(listenerEvent.getTableParameters()).thenReturn(Collections.emptyMap());
    boolean filter = listenerEventFilter.isFiltered(listenerEvent, UNREFERENCED);
    assertThat(filter).isFalse();
  }
