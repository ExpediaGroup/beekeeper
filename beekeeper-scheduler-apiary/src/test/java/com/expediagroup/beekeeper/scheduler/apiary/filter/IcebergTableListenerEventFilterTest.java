...
public class IcebergTableListenerEventFilterTest {
  ...
  @Test
  public void shouldFilterWhenFormatEqualsIcebergParquet() {
    ListenerEvent event = createListenerEventWithFormat("IcEBERG/PaRQUET");
    boolean isFiltered = filter.isFiltered(event, LifecycleEventType.EXPIRED);
    assertThat(isFiltered).isTrue();
  }

  @Test
  public void shouldNotFilterWhenFormatDoesNotEqualIcebergParquet() {
    ListenerEvent event = createListenerEventWithFormat("HIVE/ORC");
    boolean isFiltered = filter.isFiltered(event, LifecycleEventType.EXPIRED);
    assertThat(isFiltered).isFalse();
  }

  @Test
  public void shouldFilterWhenFormatEqualsIcebergParquetIgnoreCase() {
    ListenerEvent event = createListenerEventWithFormat("IcEbErG/PaRqUEt");
    boolean isFiltered = filter.isFiltered(event, LifecycleEventType.EXPIRED);
    assertThat(isFiltered).isTrue();
  }
  ...
  private ListenerEvent createListenerEventWithFormat(String format) {
    Map<String, String> tableParameters = new HashMap<>();
    tableParameters.put("format", format);
    return createListenerEventWithTableParameters(tableParameters);
  }
