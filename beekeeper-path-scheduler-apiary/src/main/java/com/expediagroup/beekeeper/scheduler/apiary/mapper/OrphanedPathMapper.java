...
public class OrphanedPathMapper extends MessageEventMapper {
  ...
  protected List<EventModel> generateEventModels(ListenerEvent listenerEvent) {
    Boolean tableWatchingUnreferenced = checkIfTablePropertyExists(listenerEvent.getTableParameters());
    ...
    case ALTER_PARTITION:
      ...
      if (!isMetadataUpdate(alterPartitionEvent.getOldPartitionLocation(),
    ...
    case ALTER_TABLE:
      ...
      break;
    case DROP_TABLE:
      eventPaths.add(new EventModel(lifecycleEventType, ((DropTableEvent) listenerEvent).getTableLocation()));
      break;
    case DROP_PARTITION:
      eventPaths.add(new EventModel(lifecycleEventType, ((DropPartitionEvent) listenerEvent).getPartitionLocation()));
      break;
    }
