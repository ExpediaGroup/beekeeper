...
- Added additional checks for the number of levels in table and partition paths so that invalid paths don't get
...
- Return Slice instead of Page in queries to avoid scanning the whole table for the total number of pages.
  Details [here](https://www.baeldung.com/spring-data-jpa-pagination-sorting#paginate).
...
- If a table gets deleted before Beekeeper is scheduled to do so, its entries in the `housekeeping_metadata` table will
  be disabled.
...
- `MeterRegistry` to `GraphiteMeterRegistry` so that Spring metrics use Beekeeper's `GraphiteConfig` and not the default
  config.
