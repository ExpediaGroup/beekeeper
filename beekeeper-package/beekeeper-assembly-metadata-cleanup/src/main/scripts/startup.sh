...
if [[ ! -z $BEEKEEPER_CONFIG ]]; then
  ...
  java -jar ./lib/beekeeper-metadata-cleanup-app.jar --config=./conf/beekeeper-config.yml $db_password
else
  java -jar ./lib/beekeeper-metadata-cleanup-app.jar $db_password
fi
