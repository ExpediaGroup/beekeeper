...
public class LifecycleEventTypeTest {
  ...
  @Test
  public void shouldValidateExpiredParameterTrue() {
    assertThat(EXPIRED.getBoolean(Map.of(EXPIRED.getTableParameterName(), TRUE_STR))).isTrue();
  }

  @Test
  public void shouldValidateExpiredParameterFalse() {
    assertThat(EXPIRED.getBoolean(Map.of(EXPIRED.getTableParameterName(), FALSE_STR))).isFalse();
  }

  @ParameterizedTest
  @ValueSource(strings = { "", " ", "beekeeper.remove.expire.data", "some.foo" })
  public void shouldIgnoreInvalidExpiredParameters(String param) {
    assertThat(EXPIRED.getBoolean(Map.of(param, TRUE_STR))).isFalse();
  }
}
