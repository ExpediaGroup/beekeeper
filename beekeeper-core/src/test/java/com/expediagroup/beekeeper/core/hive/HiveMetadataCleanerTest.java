...
public class HiveMetadataCleanerTest {
  ...
  public void typicalMetadataCleanup() {
    when(hiveClient.deleteMetadata(DATABASE, TABLE_NAME)).thenReturn(true);
    cleaner.cleanupMetadata(housekeepingMetadata);
    verify(housekeepingMetadata).getDatabaseName();
    verify(housekeepingMetadata).getTableName();
    verify(hiveClient).deleteMetadata(DATABASE, TABLE_NAME);
  ...
  public void typicalPartitionDrop() {
    ...
    cleaner.cleanupPartition(housekeepingMetadata);
    verify(housekeepingMetadata).getDatabaseName();
    verify(housekeepingMetadata).getTableName();
  ...
  public void dontReportWhenTableNotDeleted() {
    when(hiveClient.deleteMetadata(DATABASE, TABLE_NAME)).thenReturn(false);
    cleaner.cleanupMetadata(housekeepingMetadata);
    verify(hiveClient).deleteMetadata(DATABASE, TABLE_NAME);
