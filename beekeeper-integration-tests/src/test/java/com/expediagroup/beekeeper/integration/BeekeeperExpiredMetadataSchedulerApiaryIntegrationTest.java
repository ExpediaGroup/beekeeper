...
@Testcontainers
public class BeekeeperExpiredMetadataSchedulerApiaryIntegrationTest
  ...
  protected static final String PARTITION_KEYS =
      "{ \"event_date\": \"date\", \"event_hour\": \"smallint\"}";
  ...
  @RegisterExtension
  protected ThriftHiveMetaStoreJUnitExtension thriftHiveMetaStore =
  ...
  public static void init() {
    ...
    System.setProperty(
        SQS_ENDPOINT_PROPERTY, ContainerTestUtils.awsServiceEndpoint(SQS_CONTAINER, SQS));
  ...
  @Test
  public void expiredMetadataCreateTableEvent()
  ...
  @Test
  public void expiredMetadataAlterTableEvent()
      throws SQLException, IOException, URISyntaxException {
  ...
  public void expiredMetadataAddPartitionEvent()
      throws SQLException, IOException, URISyntaxException {
    AddPartitionSqsMessage addPartitionSqsMessage =
        new AddPartitionSqsMessage(LOCATION_A, PARTITION_KEYS, PARTITION_A_VALUES, true);
  ...
  @Test
  public void expiredMetadataMultipleAddPartitionEvents()
      throws SQLException, IOException, URISyntaxException {
    AddPartitionSqsMessage addPartitionSqsMessage =
        new AddPartitionSqsMessage(LOCATION_A, PARTITION_KEYS, PARTITION_A_VALUES, true);
    AddPartitionSqsMessage addPartitionSqsMessage2 =
        new AddPartitionSqsMessage(LOCATION_B, PARTITION_KEYS, PARTITION_B_VALUES, true);
    amazonSQS.sendMessage(sendMessageRequest(addPartitionSqsMessage.getFormattedString()));
  ...
  @Test
  public void expiredMetadataAlterPartitionTableEvent()
      throws SQLException, IOException, URISyntaxException {
    insertExpiredMetadata(LOCATION_A + "-old", PARTITION_A_NAME);

    AlterPartitionSqsMessage alterPartitionSqsMessage =
        new AlterPartitionSqsMessage(LOCATION_A, PARTITION_KEYS, PARTITION_A_VALUES, true);
  ...
  @Test
  public void expiredMetadataMultipleAlterPartitionTableEvents()
      throws SQLException, IOException, URISyntaxException {
    insertExpiredMetadata(LOCATION_A + "-old", PARTITION_A_NAME);
    ...
    AlterPartitionSqsMessage alterPartitionSqsMessage2 =
        new AlterPartitionSqsMessage(LOCATION_B, PARTITION_KEYS, PARTITION_B_VALUES, true);
    amazonSQS.sendMessage(sendMessageRequest(alterPartitionSqsMessage.getFormattedString()));
  ...
  @Test
  public void expiredMetadataCreateIcebergTableEvent()
      throws SQLException, IOException, URISyntaxException {
  ...
  public void testEventAddedToHistoryTable() throws SQLException, IOException, URISyntaxException {
    ...
    await()
        ...
        .until(() -> getBeekeeperHistoryRowCount(EXPIRED) == 1);
  ...
  void scheduleMissingPartitionsWhenPropertiesExpireInTable() throws Exception {
    ...
    insertExpiredMetadata(
        PARTITION_ROOT_PATH + PARTITION_A_NAME + "/event_type=C",
        PARTITION_A_NAME + "/event_type=C");

    AlterTableSqsMessage alterTableSqsMessage = new AlterTableSqsMessage(PARTITION_ROOT_PATH, true);
    amazonSQS.sendMessage(sendMessageRequest(alterTableSqsMessage.getFormattedString()));

    await().atMost(TIMEOUT, TimeUnit.SECONDS).until(() -> getExpiredMetadataRowCount() == 4);
  ...
  protected void assertExpiredMetadata(
      HousekeepingMetadata actual, String expectedPath, String partitionName) {
    assertHousekeepingMetadata(actual, expectedPath, partitionName);
    assertMetrics();
  }

  public void assertHousekeepingMetadata(
      HousekeepingMetadata actual, String expectedPath, String expectedPartitionName) {
    ...
    assertThat(actual.getCreationTimestamp())
        .isAfterOrEqualTo(CREATION_TIMESTAMP_VALUE.withNano(0));
    assertThat(actual.getModifiedTimestamp())
        .isAfterOrEqualTo(CREATION_TIMESTAMP_VALUE.withNano(0));
    assertThat(actual.getCleanupTimestamp())
        .isEqualTo(actual.getCreationTimestamp().plus(actual.getCleanupDelay()));
  ...
  public void assertMetrics() {
    Set<MeterRegistry> meterRegistry =
        ((CompositeMeterRegistry) BeekeeperSchedulerApiary.meterRegistry()).getRegistries();
